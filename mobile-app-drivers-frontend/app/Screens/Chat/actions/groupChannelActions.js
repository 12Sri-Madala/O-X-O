...
import {
  messageOwner,
} from '../../Dash/Redux/actions';


export const messageTeamOXO = () => (dispatch) => {
  dispatch(messageOwner('b7b7697f-e532-467d-aab3-397989aade4c'));
};
...
export const groupChannelProgress = start => ({ type: start ? GROUP_CHANNEL_PROGRESS_START : GROUP_CHANNEL_PROGRESS_END });
...
export const getGroupChannelList = groupChannelListQuery => (dispatch) => {
  if (groupChannelListQuery.hasNext) {
    sbGetGroupChannelList(groupChannelListQuery)
      ...
      .catch(() => dispatch({ type: GROUP_CHANNEL_LIST_FAIL }));
  } else {
