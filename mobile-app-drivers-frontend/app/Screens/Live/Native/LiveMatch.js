...
import { Spacing, Font, Colors } from "../../Library/Native/StyleGuide";
import { openMaps } from "../../Dash/Native/MatchFoundHelpers";
...
const CAR_DEFAULT_LATITUDE = 37.741895;
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.connection) {
      const room = drone.subscribe(
        `${this.props.connection.id}_${this.props.connection.ownerID}`
      );
      room.on("open", error => {
        if (error) {
