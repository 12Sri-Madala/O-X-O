...
class MatchDetails extends React.Component {
  ...
  confirm() {
    ...
    if (this.props.connection.status === "CONFIRMED") {
      this.props.dispatchUpdateConnectionStatus(
        this.state.id,
        this.state.token,
    ...
    if (
      ...
      // Doorbell feedback service requires an email, but we do not collect email information from owners. Works fine now, but solutions...
      fetch(
        `https://doorbell.io/api/applications/10636/submit?key=Uw4lyXnvngnNRIT6gamN0skHveEm3KXIb2WKXlA6H9EAnjaAptOVv83REBEPVAHK`,
        {
          method: "POST",
          headers: {
  ...
  cancel() {
    if (this.props.connection.status === "CONFIRMED") {
      ...
      Alert.alert(
        "Live Match",
        "You can't cancel a live match, please contact your driver or OXO if you need your vehicle returned.",
        [{ text: "Close", onPress: () => { } }]
      );
    }
  }
  ...
  getDriver() {
    const driver = _.find(this.props.drivers, driver => {
      return driver.id === this.props.connection.driverID;
    });
    return driver;
  }
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <DriverDetails driver={this.props.driver} car={this.props.vehicle} />
        ...
        <Modal isVisible={this.state.isRatingModalVisible}>
          <View style={{ ...StyleSheet.flatten(styles.modalContent) }}>
            <Text style={{ fontSize: Font.title_3, color: Colors.primary }}>
              Tell us how your match was!
            </Text>
            <StarRating
              disabled={false}
...
const mapStateToProps = state => {
  return {
    driver: state.live.driver,
    vehicle: state.live.vehicle,
    loading: state.live.loading,
    owner: state.live.owner
  };
};
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    dispatchUpdateConnectionStatus: (
      userId,
      token,
      connectionID,
      connectionStatus,
