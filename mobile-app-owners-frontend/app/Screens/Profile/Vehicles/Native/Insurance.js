...
import {StyleSheet, Text, View, ScrollView, ImageBackground, StatusBar, Button, Image, TouchableOpacity, TouchableHighlight,
  ...
  class Insurance extends React.Component {
    ...
    async editImage() {
      ...
      let result;
      Alert.alert(
        'Select photo of your insurance',
        '',
        [
          {
            text: 'Photo Library', onPress: async () => {
              ...
              if (!result.cancelled) {
                await this.props.dispatchSaveLicense({insuranceImage: 'data:image/png;base64,' + result.base64}, this.state.index);
              }
            }
          },
          {
    ...
    render() {
      return (
        ...
        <Text style={styles.title}>{'Upload insurance photo'}</Text>
        </View>
        {this.props.vehicles[this.state.index] &&
          <View style={{flex: 6, width: '100%', marginTop: Spacing.small}}>
          {this.props.vehicles[this.state.index].insuranceImage && (!this.props.vehicles[this.state.index].approved) ?
          ...
          {this.props.vehicles[this.state.index].insuranceImage ?
            <ImageBackground
              source={{uri: this.props.vehicles[this.state.index].insuranceImage}}
              resizeMode={'center'}
              style={styles.images}
            >
            <Icon
          ...
          Get your vehicle onboard with a photo of your insurance card. In the photo, make sure not to cut anything off!
          </Text>
    ...
    export default connect(mapStateToProps, mapDispatchToProps)(withErrorHandling(Insurance));
