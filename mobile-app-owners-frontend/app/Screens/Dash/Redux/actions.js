    ...
    try {
      fetch(`${serverInfo.name}/dash/${id}/${token}`)
        ...
        .then(responseJson => {
          ...
          const owner = responseJson.data.owner[0];

          // Sendbird information should live somewhere else or elsewise be decomposed
          sbConnect(owner.id, `${owner.firstName} ${owner.lastName}`).then(
            () => {
