...
import {View, FlatList, TouchableHighlight, TouchableOpacity, Text, Alert, Dimensions} from 'react-native';
...
class GroupChannel extends Component {
    ...
    return {
      ...
      headerLeft: (
        <Button
          ...
          buttonStyle={{paddingLeft: Spacing.small}}
    ...
    return (
      <View style={styles.renderTitleViewStyle}>
        <View style={{flexDirection: 'row'}}>
          <Text style={{fontSize: Font.large}}>{sbGetChannelTitle(channel)}</Text>
  ...
  _renderList = (rowData) => {
    ...
    if (sbGetChannelTitle(channel) === undefined) {
      return;
    }
    return (
      <Swipeout
        ...
        <ListItem
          ...
          key={channel.url}
          avatar={<Avatar title={sbGetChannelTitle(channel).split(' ')[0][0] + sbGetChannelTitle(channel).split(' ')[1][0]} />}
        ...
        <View style={{
          ...
          <View style={styles.header}>
            <Header
              icon='chevron-left'
              title= {'Messages'}
              _callback={() => this.props.navigation.navigate('Dashboard')}
              _call={() => {
            ...
            <View>
              ...
              <View style={styles.buttonContainer}>
                <TouchableOpacity
                  style={styles.button}
                  onPress={this.props.messageTeamOXO}
                  >
...
function mapStateToProps( state )  {
  const { isLoading, list, channel } = state.groupChannel;
...
)(withErrorHandling(GroupChannel));
